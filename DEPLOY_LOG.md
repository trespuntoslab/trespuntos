...
- **Origen:** auditoría de Jordan por el bridge. Verifiqué el hallazgo antes de actuar (patrón: cruzar siempre contra código + datos reales antes de ejecutar lo que proponga otro agente):
  - **Confirmado real:** post de dic-2025 con slug `mejores-practicas-desarrollo-web-live-coding-mejores-practicas-en-desarrollo-web-2026` (contiene "desarrollo-web" 2 veces) pero H1/contenido 100% sobre "7 Errores de UX/UI que destruyen el ROI". Además el `article-category` en la propia página decía "Desarrollo Web" y la card en `/blog/index.html` tenía un title completamente distinto ("Mejores prácticas en desarrollo web 2026") — 3 señales contradictorias apuntando todas a "desarrollo web" con contenido real de UX/UI.
  - **Matizado con datos GSC reales** (`/api/keyword-plan`): "desarrollo web barcelona" no está en caída libre — mejoró de pos. 13.9 (baseline 1-jun) a 8.2 (pico 15-jun), y se ha desinflado a 10.3 (6-jul). Regresión suave de 3 semanas, no colapso. La corrección se hace igualmente porque la URL es mala práctica SEO independientemente del peso causal exacto.
  - **Descartada la propuesta original de Jordan** (noindex + 301 hacia otro artículo UX/UI existente): no hay ningún post en el blog con el ángulo "errores UX/UI que destruyen el ROI", así que fusionar/noindexar habría tirado contenido único. Se optó por renombrar + 301 en su lugar (conserva el contenido bajo una URL coherente).
  - **Ya resuelto, no hacía falta tocar** (contra lo que pedía el punto 2 del informe): los CTAs internos de `desarrollo-web-a-medida-vs-wordpress` y `desarrollo-web-a-medida-cuando-es-la-decision-correcta` ya enlazan bien al servicio con buen anchor text.
- **Cambios:** slug renombrado `/blog/mejores-practicas-desarrollo-web-live-coding.../` → `/blog/errores-ux-ui-que-destruyen-el-roi/`. 301 en `.htaccess` (bloque "Renamed slugs"). `article-category` y card del blog corregidos a "UX/UI". `dateModified`/`article:modified_time` bump a hoy (se preserva `datePublished` original dic-2025). Sitemap actualizado. OG regenerada con badge `BLOG · UX/UI`, la vieja borrada.
- **Bug adicional encontrado y arreglado de paso:** meta `description` y `og:description` tenían comillas sin escapar (`web "bonita"?`) que rompían el parseo del atributo HTML — el generador de OG leía el subtítulo cortado a mitad de frase. Escapadas a `&quot;`. `twitter:title`/`twitter:description` también estaban truncados por el mismo motivo — corregidos.
- **Archivos FTP (5):** `blog/errores-ux-ui-que-destruyen-el-roi/index.html` (nuevo) · `img/og/blog-errores-ux-ui-que-destruyen-el-roi.png` (nuevo) · `blog/index.html` · `sitemap.xml` · `.htaccess`. Borrados del servidor: carpeta vieja + OG vieja (ya no están en git).
- **Orden de ejecución (importante):** subida FTP → verificado el 301 funcionando en origen (`cf-cache-status: MISS`) **antes** de borrar los archivos viejos del servidor, para no dejar ventana de 404.
- **Cloudflare:** Purge by URL — 5 URLs (vieja, nueva, OG, `/blog/`, sitemap) → `{"success": true}`.
- **Verificación (cache-bust) en producción:** URL vieja → 301 limpio ✅ · URL nueva 200 con H1/categoría correctos ✅ · OG sirviendo ✅ · card corregida en `/blog/` ✅ · sitemap solo con URL nueva ✅.
- **Pendiente:** responder a Jordan por el bridge con las correcciones (Revelación 1 no es nueva pero el bug de Curry sí es útil de arreglar; punto 2 de su informe ya estaba resuelto; acción ejecutada fue rename+301 en vez de noindex+301-a-otro-artículo).
