...
IMPORTANTE: Respuestas cortas y naturales. 2-4 frases. Esto es un chat, no un email.`;
  ...
  class JordanWidget {
    ...
    open() {
      ...
      // Body scroll lock — lock both html and body
    ...
    _setupMobileKeyboard() {
      ...
      // Direct inline style approach — no CSS variables, no calc()
      // Just set height = visualViewport.height px whenever it changes
      const applyHeight = () => {
      ...
      const resetHeight = () => {
        this.chatEl.style.setProperty('height', '100vh', 'important');
        this.chatEl.style.setProperty('max-height', '100vh', 'important');
      ...
      // Focus: ensure scroll after keyboard animation
      this.textarea.addEventListener('focus', () => {
        if (window.innerWidth > 768 || !this.isOpen) return;
        // Wait for keyboard to finish opening, then apply
        setTimeout(() => {
          applyHeight();
          this._scrollToBottom();
        }, 350);
      ...
      // Blur: reset to full height when keyboard closes
      ...
      // iOS 26: force viewport reset after keyboard dismissal
