...
IMPORTANTE: Respuestas cortas y naturales. 2-4 frases. Esto es un chat, no un email.`;
  ...
  const WIDGET_CSS = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @media (max-width: 768px) {
      ...
      /* Hide footer when keyboard is open to save space */
      .jordan-chat.keyboard-open .jordan-footer {
        display: none;
      }

      /* Reduce input padding when keyboard is open */
      .jordan-chat.keyboard-open .jordan-input-area {
        padding-top: 8px;
        padding-bottom: 8px;
      }

      /* Cap textarea height when keyboard open */
      .jordan-chat.keyboard-open .jordan-textarea {
        max-height: 60px;
      }

      /* Smooth height transition when keyboard opens/closes */
      .jordan-chat.open {
        transition: height .2s ease, max-height .2s ease, top .15s ease,
  ...
  class JordanWidget {
    ...
    open() {
      ...
      // Body scroll lock — prevent background scroll without position:fixed hack
      ...
      if (window.innerWidth <= 768) {
        // Use touch-action + overscroll-behavior instead of position:fixed
        // position:fixed causes conflicts with visualViewport on iOS
    ...
    close() {
      ...
      // Reset chat inline styles set by keyboard handler
      if (window.innerWidth <= 768) {
    ...
    _setupMobileKeyboard() {
      ...
      // Apply chat height to match visible viewport (keyboard-aware)
      const applyHeight = () => {
        ...
        // Detect keyboard state: viewport shrunk significantly = keyboard open
        const fullHeight = window.innerHeight;
        const isKB = h < fullHeight * 0.85;
        if (isKB !== this._keyboardOpen) {
          this._keyboardOpen = isKB;
          this.chatEl.classList.toggle('keyboard-open', isKB);
        }

        requestAnimationFrame(() => this._scrollToBottom());
      };

      // Reset to full viewport when keyboard closes
      const resetHeight = () => {
        const dvh = CSS.supports && CSS.supports('height', '100dvh') ? '100dvh' : '100vh';
        this.chatEl.style.setProperty('height', dvh, 'important');
        this.chatEl.style.setProperty('max-height', dvh, 'important');
        this.chatEl.style.setProperty('top', '0px', 'important');
        this._keyboardOpen = false;
        this.chatEl.classList.remove('keyboard-open');
      };

      // Stabilized apply — waits for viewport to stop changing (keyboard animation done)
      const stabilizedApply = () => {
        clearTimeout(stabilizeTimer);
        applyHeight();
        // Re-check after keyboard animation might still be running
        stabilizeTimer = setTimeout(() => {
          if (this.isOpen && Math.abs(vv.height - lastVVHeight) > 2) {
      ...
      vv.addEventListener('resize', stabilizedApply);
      vv.addEventListener('scroll', () => {
        if (!this.isOpen) return;
        // Only adjust top offset on viewport scroll (iOS address bar)
        const top = Math.round(vv.offsetTop);
        this.chatEl.style.setProperty('top', top + 'px', 'important');
      });

      // Focus: wait for keyboard animation, then apply + scroll
      this.textarea.addEventListener('focus', () => {
        if (window.innerWidth > 768 || !this.isOpen) return;
        // Staggered apply: fast initial + delayed settle
        setTimeout(() => { applyHeight(); this._scrollToBottom(); }, 100);
        setTimeout(() => { applyHeight(); this._scrollToBottom(); }, 350);
        setTimeout(() => { applyHeight(); this._scrollToBottom(); }, 600);
      });

      // Blur: reset when keyboard fully closes
      ...
      // iOS: force viewport reset after keyboard dismissal
      ...
      // Prevent iOS bounce — allow scroll in messages, textarea, AND quick reply buttons
      this.chatEl.addEventListener('touchmove', (e) => {
        const messages = this.messagesEl;
        if (!messages) return;
        const target = e.target;
        // Allow scroll in messages, textarea, quick replies, and file preview
        if (messages.contains(target) || this.textarea.contains(target)) return;
    ...
    _scrollToBottom() {
      requestAnimationFrame(() => {
        // Use scrollIntoView on last message for smoother mobile behavior
